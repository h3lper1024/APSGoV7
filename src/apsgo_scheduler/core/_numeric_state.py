...
_WIDTH_PARAMETERS = ('maximum_increase', 'max_reverse_width', 'virtual_width_tolerance',
                     'width_upper_exclusive')
_WEIGHT_PARAMETERS = ('max_weight', 'min_weight', 'target_weight', 'max_real_weight',
                      'future_fill_weight_target', 'maximum_piece_weight',
                      'minimum_piece_weight', 'maximum_separator_weight')
...
def _text(material, name):
    ...
    return value.strip().upper() if name in ('grade','hot_roll_grade','surface_grade') else value.strip()
...
class NumericNodeColumns:
    ...
    def __post_init__(self):
        ...
        for field in fields(self):
            ...
            dtype = np.bool_ if field.name == 'present' else np.int8 if field.name == 'role' else np.int64
...
class NumericSplitGroups:
    ...
    def __post_init__(self):
        ...
        for field in fields(self):
            value = getattr(self, field.name)
            if (not isinstance(value, np.ndarray) or value.dtype != np.int64 or value.shape != (size,)
                    or value.flags.writeable or not value.flags.c_contiguous):
                raise NumericValueError(f'split_groups.{field.name}', 'read-only integer column required')
...
class NumericTask:
    ...
    def build(cls, problem, rule_set, timing_input=None):
        ...
        if timing_input is None and (problem.delivery_timing is not None or any(
                type(rule).__name__ == 'DeliveryDuePerformanceRule' for rule in rule_set.rules)):
        ...
        for name, scale in zip(_PHYSICAL, scales):
            values[name] = readonly([0 if getattr(node, name) is None else
                to_ticks(getattr(node, name), scale, f'nodes[{i}].{name}')
                for i, node in enumerate(materials)], np.int64)
        for name in ('width', 'thickness'):
            column = values[name]
            int64(int(column.max()) - int(column.min()), f'{name}.difference_bound')
        temperatures = [int(values[name][i]) for i,node in enumerate(materials)
                        for name in ('min_temperature','max_temperature') if getattr(node,name) is not None] or [0]
        ...
        weights = [to_ticks(node.weight, units.weight, f'orders[{i}].weight')
                   for i, node in enumerate(problem.nodes)]
        weights += [to_ticks(node.unit_weight, units.weight, f'prototypes[{i}].weight')
        ...
        values['role'] = readonly([_ROLES.index(node.material_role) for node in problem.nodes]
                                 + [_ROLES.index(MaterialRole.GENERATED_VIRTUAL)] * len(prototype_ids), np.int8)
        values['source_period'] = readonly([periods[node.source_period] for node in problem.nodes]
                                          + [-1] * len(prototype_ids), np.int64)
        for key in ('source', 'resource'):
            values[key] = readonly([*range(original_count), *([-1] * len(prototype_ids))], np.int64)
        values['prototype'] = readonly([-1] * original_count + list(range(len(prototype_ids))), np.int64)
        ...
        if timing_input is not None:
            ...
            rates = {key.strip(): value for key, value in timing_input.virtual_hours_per_tonne.items()}
            if (len(by_source) != len(timing_input.orders) or set(by_source) != set(source_ids)
                    or len(rates) != len(timing_input.virtual_hours_per_tonne) or set(rates) != set(prototype_ids)):
                raise NumericValueError('timing', 'timing must cover each original and prototype exactly once')
            start = start_milliseconds(timing_input.schedule_start_at, 'schedule_start_at')
            for i, source in enumerate(source_ids):
                due[i] = due_milliseconds(by_source[source].due_date, start, f'orders[{i}].due_date')
                durations[i] = hours_to_milliseconds(by_source[source].duration_hours, f'orders[{i}].duration')
            for i, prototype in enumerate(problem.virtual_prototypes):
                durations[original_count+i] = hours_to_milliseconds(
                    rates[prototype.prototype_id], f'prototypes[{i}].duration', weight=prototype.unit_weight)
            ...
            checked_product(checked_sum(weights[:original_count], 'original_weight_sum'),
                            maximum_wait_seconds, 'original_weight_time_bound')
        values['duration_ms'] = readonly(durations, np.int64)
        originals = NumericOriginals(values['weight'][:original_count],
        ...
        for rule in rule_set.rules:
            ...
            if name in ('StrategicCustomerPriorityRule', 'SyntheticNodePriorityRule'):
                ...
                priority.append([int64(rule.construction_priority(node)[0], rule.rule_id)
                                 for node in problem.nodes] + [0] * len(prototype_ids))
                ...
                narrow.append([i < original_count and values['role'][i] == 0 and present[i][0]
                               and values['width'][i] < limit and _text(node, 'grade_class').strip() == parameters['grade_class']
                               for i, node in enumerate(materials)])
            elif name == 'HighSurfaceRunCountRule':
                surface_ids.append(rule.rule_id)
                configured_grades = {grade.strip().upper() for grade in parameters['surface_grades']}
                surface.append([i < original_count and values['role'][i] == 0
                ...
                for i in range(count):
                    if i >= original_count or values['role'][i] != 0:
                        column.append(-1)
                        continue
                    key = tuple((present[i][_PHYSICAL.index(field)], int(values[field][i]))
                                if field in _PHYSICAL else int(values[field][i])
                                for field in parameters['group_by_fields'])
                    column.append(codes.setdefault(key, len(codes)))
                groups.append(column)
        derived = [readonly(np.asarray(rows, dtype=dtype).reshape((len(rows), count)), dtype)
                   for rows, dtype in ((priority,np.int64),(narrow,np.bool_),(surface,np.bool_),(groups,np.int64))]
        nodes = NumericNodeColumns(**values)
        digest = sha256()
        digest.update(fingerprint({'units': units.fingerprint, 'problem': problem.input_fingerprint,
                                  'rules': rule_set.fingerprint, 'text': text_labels,
                                  'start_ms': start}).encode('ascii'))
        ...
        for index, array in enumerate((originals.due_ms, originals.old_backlog, *derived)):
            ...
            digest.update(array.astype(array.dtype.newbyteorder('<'), copy=False).tobytes())
        return cls(units, nodes, originals, NumericSplitGroups.empty(),
...
class NumericPlan:
    ...
    def build(cls, task, node_rows, chain_offsets, chain_ids, chain_periods, *, generation=0):
        ...
        rows, offsets, ids, periods = (_indices(value,name) for value,name in
            ((node_rows,'node_rows'),(chain_offsets,'chain_offsets'),(chain_ids,'chain_ids'),
        ...
        if np.any(ids < 0) or np.unique(ids).size != chains:
            raise NumericValueError('chain_ids','chain identities must be unique nonnegative integers')
        ...
        if virtual_rows.size and (np.any(task.nodes.purpose[virtual_rows] < 0)
        ...
        source_offsets = np.concatenate((np.zeros(1,dtype=np.int64),np.cumsum(source_counts,dtype=np.int64)))
        for source in range(original_count):
            selection = pieces[source_offsets[source]:source_offsets[source+1]]
            if checked_sum((int(w) for w in task.nodes.weight[selection]),'source_weight') != int(task.originals.weight[source]):
                raise NumericValueError('source_weight','original weight is not conserved')
            if checked_sum((int(d) for d in task.nodes.duration_ms[selection]),'source_duration') != int(task.originals.duration_ms[source]):
        ...
        row_chain,row_position = np.full(capacity,-1,dtype=np.int64),np.full(capacity,-1,dtype=np.int64)
