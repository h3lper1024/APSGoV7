...
用户本轮确认虚拟材统一 **100 米/分钟**，客户端和接口适配暂不考虑，先完成后端算法。前述待确认状态为历史记录，阶段 0 现已完成来源/参数冻结，不代表工时计算或搜索代码完成。

- 继续使用第 3 节全部源文件及哈希；原订单身份、重量、期序和顺序取旧 `prepared_request.json`，交期/炉速通过原订单号读取 Excel 的 AY/CH 列，不用已拆片结果作新订单。
- 虚拟工时使用每个冻结原型自身宽厚和明确 100 米/分钟；在后端请求准备时推导，不写库、不改原始原型快照。
- 开始时间 `2026-06-01T00:00:00+08:00`；截止当天 24 点；种子 590531、200000 次、310 秒总时限及 10 秒收尾不变。
- 旧参考保持第 2.2 节七级身份；新请求在完成后端准备时另生成计时身份和九级规则副本。不得覆写旧请求和旧结果。
- 本轮阶段 5 只接后端 Python 入口和重放工具，阶段 6 做后端真实比较，阶段 7 做源码回归。HTTP/前端/Windows 待后续适配，不在本轮修改或验收。
- 本次只复核源哈希、文档及提交范围，不重复跑业务测试。实际提交/检查记录见本次 Git 正文；随后继续阶段 1～2。
