...
1. 阶段 6 实施时，`ApsgoV7Configuration` 严格读取 V7 基础地址、月计划求解路径和 240 秒客户端超时；规则客户端只读取基础地址，不依赖求解专属键。
...
后续配置补正（2026-09-07）：后端搜索时限提高为 300 秒、总时限提高为 310 秒后，C# 提交 `ae8587f` 将当前等待超时提高为 370 秒。本文其余内容继续记录阶段 6 当时的实现和验证事实。
