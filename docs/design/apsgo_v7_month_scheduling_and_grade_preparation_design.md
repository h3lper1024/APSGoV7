...
此前 180 秒完整流程性能门槛和 `run_01`～`run_05` 仍是历史验收口径，不改写旧结果；它们不能证明当前 310 秒运行策略满足原 180 秒门槛。新策略的独立 `run_06` 已在 222.596230 秒达到 200000 次候选检查上限并通过质量与双审计，搜索并未用满 300 秒；该单样本不代替多次性能验收，也不能据此修改历史门槛。
