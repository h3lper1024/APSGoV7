...
> 本文描述目标设计，不表示 HTTP 服务、数据库表或 C# 页面已经完成改造。
...
- `compute_rule_set_fingerprint()`：根据规范化内容计算规则集身份。
...
| `401` / `403` | 现有宿主鉴权判定未登录或无权限 | 交给现有登录/权限机制处理；本专项不自建鉴权体系。 |
...
1. 校验请求结构并规范化可编辑内容，数值直接解析为 Python `Decimal`，不得先经过二进制浮点数再计算指纹。
...
数据库引擎和现有命名规范在实施阶段现场确认；本节定义必须满足的逻辑字段与约束，不提前绑定某一种 ORM。
...
| `remark` | 用户填写的变更说明。 |
| `created_by` / `created_at` | 由服务端可信上下文和服务器时间生成。 |
| `activated_at` | 事务启用时间。 |
...
宽度、厚度、重量、比例和温度等业务数值进入 V7 契约前使用 `Decimal`。HTTP JSON 数值的词法值应直接解析为 `Decimal`；规范化 JSON 使用稳定字段顺序和稳定十进制表示。这样同一业务内容不会因 `float` 二进制误差产生不同指纹。
...
新增独立 V7 客户端，例如 `SchedApp/ApsgoV7RuleApiClient.cs`，并增加独立的 V7 服务地址配置。不得直接删除或整体改名 `PipelineV3ApiClient.cs`，因为其他页面仍可能依赖 V3 客户端。
...
- `created_by` / `activated_by` 必须来自现有宿主的可信登录上下文，不能把请求体中的用户名当作可信身份。
- 若当前宿主尚无服务端身份，本期实现前必须明确现有鉴权传递方式；可显示的客户端用户名只能作为非可信备注，不能替代审计主体。
- POST 接口执行现有权限检查；GET 至少要求现有业务访问权限。具体权限代码复用现有月计划系统，不在 V7 核心中自建用户体系。
- 日志记录规则集身份、操作标识、旧/新版本、指纹、结果码和可信操作者，不记录数据库密码或完整敏感请求。
...
以下不是业务方案待选项，而是代码落点所需的现状信息：

1. 月计划接口实际由哪个服务进程承载、使用什么 HTTP 框架和部署端口。
2. 现有规则数据库的引擎、迁移工具、事务接口，以及服务端可信用户身份如何传入。

实施只适配现场确认的一套宿主和数据库，不同时建设多套框架适配层。若决定把 HTTP/数据库依赖直接放入 APSGOV7，须先更新当前仅允许 `apsgo_scheduler.api/core/app` 的架构约束；规则核心本身仍保持无 Web、ORM 或数据库依赖。
