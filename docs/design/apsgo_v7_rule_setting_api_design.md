...
| 项目 | 内容 |
|---|---|
| 文档状态 | 实施中；阶段 0～10 已完成代码与静态契约验证，下一阶段为联调与验收 |
| 文档版本 | v0.10 |
| 编写日期 | 2026-09-07 |
...
> 本文描述目标设计；规则管理契约、GQGA4 完整规则编译、SQLite 规则版本存储、保存并启用事务、两条 HTTP 路由、一次性初始化命令、求解启动绑定以及 C# 页面查询和保存流程均已实现并完成静态契约验证。本专项仍不包含月计划排程 HTTP 接口适配；Windows 构建和真实页面联调尚未完成。
...
- 用户新增宽度与厚度组合时，原型标识固定为 `virtual_sphc:{width}x{thickness}`，牌号为 `SPHC`，最小/最大温度为空，规则属性为空对象。
